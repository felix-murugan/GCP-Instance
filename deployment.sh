...
echo "DEBUG: Deployment script started at $(date)"

# ===== Load GitHub token from metadata =====
...
if [ -z "$GITHUB_TOKEN" ]; then
    echo "❌ ERROR: GitHub token not found in metadata. Exiting."
    exit 1
fi
echo "DEBUG: GitHub token found"
